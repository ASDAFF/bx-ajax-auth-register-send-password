	...
	<form name="system_auth_form<?= $arResult["RND"] ?>"
		...
		<div class="uk-form-row">
			<div class="uk-form-controls">
				...
				<?/*if($arParams['PASSWORD_LENGTH']):?>
					<div class="uk-text-small"><?=GetMessage('PASSWORD_MIN_LENGTH', array('#LENGTH#' => $arParams['PASSWORD_LENGTH']))?></div>
				<?endif*/?>
			</div>
		</div>
		...
		<div class="uk-form-row auth-links">
			<!--noindex-->
			<div>
		...
		<? if ($arResult["AUTH_SERVICES"] && COption::GetOptionString('main', 'allow_socserv_authorization', 'N') == 'Y'): ?>
			<div class="uk-form-row auth-services">
				<div class="bx-authform-title"><span><?= GetMessage("AUTH_SERVICES_LOGIN") ?></span></div>
				<?
...
file_put_contents($tttfile, "<pre>".print_r($arResult,1)."</pre>\n");
?>